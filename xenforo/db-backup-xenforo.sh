...
BACKUP_PATH=${HOME}/backups/db-backups/xenforo
ENCRYPTED_BACKUP_PATH=${HOME}/backups/encrypted-db-backups/xenforo
...
# sed removes three characters via... ';\r (apostrope, semicolon and carragi return)
DB_Name=$(/bin/sed -n "/dbname/ s/[';\r]//gp" ${ConfigFilePath} | /bin/awk '{print $3}')
DB_User=$(/bin/sed -n "/username/ s/[';\r]//gp" ${ConfigFilePath} | /bin/awk '{print $3}')
DB_Pass=`/bin/sed -n "/password/ s/[';\r]//gp" ${ConfigFilePath} | /bin/awk '{print $3}'`
